...
parser.add_argument('-file', '--filename', help='output filename', default='3D_template_bank', type=str)
...
mcmc_steps = math.ceil(total_templates_required/nwalkers)
...
with Pool(processes = ncpus) as pool:
    ...
    start = time.time()
    state = sampler.run_mcmc(initial_guess, burn_in_steps, progress=True)
    end = time.time()
...
indices = np.random.choice(len(template_bank), total_templates_required, replace=False)
final_template_bank = template_bank[indices]

template_bank_output_filename = output_path + args.filename + '_gpu_format.txt'


with open(template_bank_output_filename, 'w') as f:
    f.write(f'# TEMPLATE BANK TECHNIQUE: RANDOM\n')
    f.write(f'# TOBS (h): {args.obs_time/60}\n')
    f.write(f'# COVERAGE: {coverage}\n')
    f.write(f'# MISMATCH: {mismatch}\n')
    f.write(f'# SPIN PERIOD (ms): {fastest_spin_period_ms}\n')
    ...
    f.write(f'# ORBITAL PHASE MIN (rad): {min_initial_orbital_phase}\n')
    f.write(f'# ORBITAL PHASE MAX (rad): {max_initial_orbital_phase}\n')
