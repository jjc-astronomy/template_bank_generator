#!/homes/jjiang/anaconda3/bin/python
# -*- coding: utf-8 -*-
# =============================================================================
...
parser.add_argument('-file', '--filename', default='', type=str)
parser.add_argument('--nwalkers', type=int, help='number of walkers')
parser.add_argument('--burnin', type=int, help='Burn-in length')
parser.add_argument('--thin', type=int, help='Thinning length')
...
with Pool(processes=ncpus) as pool:
    ...
    logger.info(f"Using burn-in of {args.burnin} and thinning of {args.thin} based on input.")
    mcmc_steps = math.ceil(args.templates / nwalkers * args.thin * 1.1 + args.burnin)
    #sampler.reset()
    start = time.time()
    sampler.run_mcmc(initial_guess, mcmc_steps, progress=True)
    logger.info("Main Phase took {:.1f} seconds".format(time.time() - start))

#sampler = emcee.backends.HDFBackend(filename, read_only=True)
template_bank = sampler.get_chain(discard=args.burnin, flat=True, thin=args.thin)
print(len(template_bank), args.templates)
...
# Generate corner plot
    samples = template_bank
    log_probs = np.log10(np.exp(sampler.get_log_prob(discard=args.burnin, flat=True, thin=args.thin)))
